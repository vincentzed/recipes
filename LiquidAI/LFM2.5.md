...
|-------|-----------|------------------------|---------|-------|-------------|
| LFM2.5 230M | 230M | 1× (any) | 128K | ✓ | [LiquidAI/LFM2.5-230M](https://huggingface.co/LiquidAI/LFM2.5-230M) |
...
gives no speedup) and the standard serve path is well optimized. We A/B-tested the common tuning
knobs with `vllm bench serve` (2000 prompts per config, each config served twice to cancel
launch-to-launch drift). The serve-to-serve variance is ~2–6%; most flags fall inside it. Two
clear it, plus one that helps a metric throughput doesn't show:
...
Knobs we measured and found **within the noise** for single-GPU LFM2.5 (not worth setting):
`VLLM_SSM_CONV_STATE_LAYOUT` (SD vs DS), `--mamba-backend` (triton vs flashinfer),
`--mamba-cache-mode` (none vs all), and `--mm-processor-cache-type` (lru vs shm).
`--renderer-num-workers > 1` fails to start on 0.23.0.
